...
(`med' <- `exp' at10_minq matSMKpreg patSMKpreg bmi_8 sex i.parentalsoc) //`if' `in'
...
postfile `memhold' 	str30 outcome str30 exposure str10 mediator N ///
					...
					using "PRS_Inflamm_Psych_NOIMP_Mediation_Results.dta", replace	
...
use "PRS_Inflamm_Psych_NOIMP_Mediation_Results.dta", clear	
foreach n in ind dir tot {
	gen `n'_OR = exp(`n'_obs_coef)
	gen `n'_lci_OR = exp(`n'_lci_norm)
	gen `n'_uci_OR = exp(`n'_uci_norm)
}
save "PRS_Inflamm_Psych_NOIMP_Mediation_Results.dta", replace	
